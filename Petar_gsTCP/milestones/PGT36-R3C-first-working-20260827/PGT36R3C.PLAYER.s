*
* PGT36 R3A SOCKET-BRIDGE CONTRACT - HARDEN-R1K1 CONTRACT
*
* Native Uthernet II STREAMFAST TCP client for Petar's Ensoniq
...
* no $00 samples
*
* NP0-SF1 uses the fixed native endpoint 192.168.7.54:49152 ->
* 192.168.5.235:22510, owns Uthernet II socket 0 in MACRAW mode,
* and feeds the existing exact producer ABI from STREAMFAST receive.
*
...
*
         ...
         use   4/Std.Macs
         use   Text.Macs.s
         use   Marinetti.Macs.D8.s
         use   Tool225.Macs.s
         use   4/Util.Macs

*=================================================
* NP0-C S16 entry trampoline.
* S16 execution begins at segment offset 0.
* Native modules are linked before Start, so offset 0 MUST branch to Start.
*=================================================
...
*=================================================
* NP0-SF1 native STREAMFAST link.
* The surrounding producer/Tool225 logic remains the golden M3R36A baseline.
*=================================================

         mx    %00
         put   native/include/PGT.EQU.S
         put   native/include/PGT.WIRE.EQU.S
         put   native/include/PGT.STREAM.EQU.S
         put   native/include/PGT.TCP.EQU.S
         put   native/link/uthernet2/U2.EQU.S
         put   native/core/PGT.CHECKSUM.S
         put   native/link/uthernet2/U2.BUS.S
         put   native/link/uthernet2/U2.MACRAW.S
         put   native/core/PGT.TCP.STATE.S
         put   PGT.SOCKET.R0.PRODUCER.S
         mx    %00
...
StackMarinetti   equ   $0000
StackNative      equ   $0001
...
ReadChunkNow
         ...
         jsr   PollNetworkStack
         jsr   GetTCPStatusDispatch
...
NetworkCleanExit
         jsr   ShutPlaybackTool
         jsr   ShutNetworkFrontendDispatch
         jsr   ReleaseAllRings
         jsr   ShutMemory
...
* skipped on this recovery path.
         ...
         pea   LowWaterReconnectMsg
         _WriteCString
         jsr   CloseNetworkSessionDispatch
         jsr   OpenNetworkStreamDispatch
...
NetworkChangeAfterStop
         ...
         _WriteCString
         jsr   PrintDiagnosticsSnapshot
         jsr   RepairPlaybackToolIfNeeded
...
D8StackSelected
         jsr   StartNetworkStack
         bcc   D8NetworkStackReady

         sta   LastNetworkError
         lda   NetStackMode
         beq   D8MarinettiStartFailed

         pea   ^NP0CStartFailMsg
         pea   NP0CStartFailMsg
         _WriteCString
         lda   LastNetworkError
         jsr   WriteHexWord
...
NetworkSessionPrompt
         ...
         pea   ^D8NativeConnectingMsg
         pea   D8NativeConnectingMsg
         _WriteCString

         jsr   OpenNetworkStreamDispatch
         bcc   D8StaticStreamOpen
...
NetworkPrefillFailed
         ...
         jsr   ReleaseAllRings
         brl   NetworkSessionPrompt
D8NativePrefillFailed
         lda   NetworkStopRequested
         bne   NetworkPrefillControl
         lda   #ActionLost
         sta   NetworkAction
         inc   DisconnectCount
         jsr   PrintNP0CPrefillFailure
         pea   ^NP0CFailKeyMsg
         pea   NP0CFailKeyMsg
         _WriteCString
         brl   NP0CFailureHalt
...
NetworkPrefillReady
* STREAMFAST hardware path continues directly into the proven M3R36A/Tool225
* handoff after the 480 KiB warm prefill.
         jsr   ResetSessionDiagnostics
         brl   InitialRingsReady

*-------------------------------------------------
* NP0-C first-failure receipt.
* Deliberately avoids the large golden snapshot path.
*-------------------------------------------------
PrintNP0CPrefillFailure
         pea   ^NP0CFailHdrMsg
         pea   NP0CFailHdrMsg
         _WriteCString
         lda   LastNetworkError
...
*-------------------------------------------------
* R2B diagnostic stop.
* No Tool call, cleanup call, stack mutation, or native call occurs
* after the first native-stage failure receipt.  Reboot after capture.
*-------------------------------------------------
NP0CFailureHalt
         bra   NP0CFailureHalt

*=================================================
* R0-PGT36 TRANSPORT-ONLY BENCHMARK
*
* Tool225 is never loaded or started.  The existing 512K mono allocation is
* used only as a circular scratch destination.  Every application operation
* is an exact 16K quantum composed from <=8K socket reads, matching the
* HARDEN-R1K1 caller contract.
*
* Diagnostics are deliberately passive:
* - hot receive path maintains only required socket head/tail/used state;
* - error counters increment only on exceptional paths;
* - total bytes advance once per completed 16K application quantum;
* - no tick reads or console output occur in the hot path;
* - D snapshots only at a completed 16K safe boundary; ESC exits there too.
*=================================================
...
R0BenchLoop
* Scratch destination = LeftRingPtr + (chunk mod 32)*$4000.
         lda   R0ScratchChunk
         and   #$0003
         xba
         asl   a
...
R0ScratchReady
         ...
         ldx   R0ScratchPtr
         ldy   R0ScratchPtr+2
         jsr   NativeReadNetworkBytes
* R3A: R2B transport/window + HARDEN exact 16K / <=8K application drain.
         bcs   R0BenchFailed

* One passive accounting update per 16K, outside TCP packet service.
         lda   R0BytesTotal
         clc
         adc   #$4000
         sta   R0BytesTotal
...
* Sample queue high-water only once per completed application quantum.
         lda   NP0CSockUsed
         cmp   R0SampleQueueHigh
         bcc   R0BenchKey
         beq   R0BenchKey
         sta   R0SampleQueueHigh

R0BenchKey
         jsr   R0CheckBoundaryKey
         bcc   R0BenchLoop
         cmp   #$0044
         beq   R0BenchSnapshotContinue
         brl   R0BenchExit

R0BenchSnapshotContinue
         jsr   R0PrintSnapshot
         brl   R0BenchLoop
...
* Return carry set only for ESC/D. A=ASCII code.
R0CheckBoundaryKey
         php
         sei
         sep   #$20
...
R0PrintSnapshot
* This is intentionally cold-path only (D, failure, exit).
         jsr   ReadSystemTick
         pea   ^R0SnapBytesMsg
         pea   R0SnapBytesMsg
         _WriteCString
...
*-------------------------------------------------
* D8 runtime TCP stack selection and service dispatch
*-------------------------------------------------
SelectNetworkStack
D8StackPromptAgain
         pea   ^D8StackPromptMsg
         pea   D8StackPromptMsg
         _WriteCString
         lda   #1
         ldx   #HostBuffer
         ...
         cmp   #$006E
         beq   D8SelectNative
         bra   D8StackPromptAgain
D8SelectMarinetti
         stz   NetStackMode
         pea   ^D8MarinettiSelectedMsg
         pea   D8MarinettiSelectedMsg
         _WriteCString
         clc
         rts
...
OpenNetworkStreamDispatch
         lda   NetStackMode
         bne   NativeOpenNetworkStream
         jsr   OpenNetworkStream
         rts
NativeOpenNetworkStream
         stz   NetSocketOpen
         jsr   PGTNativeOpen
         bcc   D8NativeOpenOK
         rts
...
GetTCPStatusDispatch
         lda   NetStackMode
         bne   NativeGetTCPStatus
         jsr   GetTCPStatus
         rts
NativeGetTCPStatus
         rep   #$30
         stz   TCPStatusBuffer
         stz   TCPStatusBuffer+2
         stz   TCPStatusBuffer+4
         stz   TCPStatusBuffer+6
         stz   TCPStatusBuffer+8
         stz   TCPStatusBuffer+10
         lda   NP0CConnected
         beq   D8NativeStatusClosed
         lda   #TCPEstablished
         sta   TCPStatusBuffer
         lda   NP0CSockUsed
         sta   TCPStatusBuffer+8
         stz   TCPStatusBuffer+10
         clc
         rts
D8NativeStatusClosed
         lda   #TCPClosed
         sta   TCPStatusBuffer
         lda   #NetErrClosed
         sta   TCPStatusBuffer+2
         clc
         rts
...
ReadNetworkBytesDispatch
         pha
         lda   NetStackMode
         bne   D8ReadNativeSelected
         pla
         jsr   ReadNetworkBytes
         rts
D8ReadNativeSelected
         pla
         jsr   NativeReadNetworkBytes
         rts

NativeReadNetworkBytes
* R0-PGT36: exact application request over a private TCP socket ring.
* Observable contract intentionally mirrors HARDEN-R1K1:
*   poll -> status queued -> read <=queued, <=8K, bank-safe -> repeat.
* No diagnostics, tick reads, keyboard polling, or Tool calls occur here.
         sta   NetRequestTotal
         sta   NetRemaining
         stx   NetReadPointer
...
R0NativeReadLoop
         lda   NetRemaining
         beq   R0NativeReadDone

         jsr   PGTNativePollCompat
         bcs   R0NativeReadFail

         lda   NP0CConnected
         beq   R0NativeReadClosed

         lda   NP0CSockUsed
         beq   R0NativeReadLoop
         cmp   NetRemaining
         bcc   R0NativeQueuedWithinRemain
         lda   NetRemaining
R0NativeQueuedWithinRemain
         cmp   #$2000
         bcc   R0NativeLenReady
         beq   R0NativeLenReady
         lda   #$2000
R0NativeLenReady
         sta   NetReadLength

* Preserve HARDEN-R1K1's 64K destination bank guard.
         lda   NetReadPointer
         beq   R0NativeBankReady
         eor   #$FFFF
         inc   a
         cmp   NetReadLength
         bcs   R0NativeBankReady
         sta   NetReadLength
R0NativeBankReady

         lda   NetReadLength
         ldx   NetReadPointer
         ldy   NetReadPointer+2
         jsr   PGTNativeReadAvailable
         bcs   R0NativeReadFail
         sta   NetReadCount
         beq   R0NativeReadLoop
...
CloseNetworkSessionDispatch
         lda   NetStackMode
         bne   NativeCloseNetworkSession
         jsr   CloseNetworkSession
         rts
NativeCloseNetworkSession
         lda   HiddenRefreshActive
         bne   D8NativeCloseKeepPlayback
         stz   PlaybackRunningFlag
D8NativeCloseKeepPlayback
         lda   NetSocketOpen
         beq   D8NativeSessionClosed
         jsr   PGTNativeClose
         stz   NetSocketOpen
D8NativeSessionClosed
         clc
         rts

ShutNetworkFrontendDispatch
         lda   NetStackMode
         bne   NativeShutNetworkFrontend
         jsr   ShutNetworkFrontend
         rts
NativeShutNetworkFrontend
         jsr   NativeCloseNetworkSession
         jsr   PGTNativeStop
         stz   TCPToolActive
         stz   TCPToolLoaded
         lda   TextToolActive
         beq   D8NativeTextNotActive
         _TextShutDown
         stz   TextToolActive
D8NativeTextNotActive
         lda   TextToolLoaded
         beq   D8NativeTextNotLoaded
         pea   TextTool
         _UnloadOneTool
         stz   TextToolLoaded
D8NativeTextNotLoaded
         clc
         rts
...
PlaybackToolNotActive
         lda   PlaybackToolLoaded
         beq   PlaybackToolAlreadyStopped
...
D8BuildIDMsg
         asc   'PGT36 R3C HARDEN-R1K1 keyfix player'
         dfb   $0D,$0A,$00
D8StackPromptMsg
         asc   'TCP stack: M=Marinetti N=Petar D7M N10 [N]: '
         dfb   $00
D8MarinettiSelectedMsg
         asc   'Stack: Marinetti Tool $36'
         dfb   $0D,$0A,$00
D8NativeSelectedMsg
         asc   'Stack: Petar_gsTCP D7M N10'
         dfb   $0D,$0A,$00
D8NativeConnectingMsg
         asc   'Connecting with Petar_gsTCP R2B socket ring...'
         dfb   $0D,$0A,$00

BannerMsg
         asc   'HARDEN-R1K1 native TCP player'
         dfb   $0D,$0A
         asc   'Tool225 raw TCP player / R2B transport'
         dfb   $0D,$0A
         asc   'ESC exit, C reconnect, R reset, D diag'
         dfb   $0D,$0A,$0D,$0A,$00
EndpointPromptMsg
...
* R0 transport benchmark cold-path state.
R0ScratchChunk    ds 2
R0ScratchPtr      ds 4
R0BytesTotal      ds 4
R0StartTick       ds 4
R0ElapsedTick     ds 4
R0SampleQueueHigh ds 2

R0NativeOnlyMsg  asc 'Transport: Petar_gsTCP R2B window=$0B68',0D,00
R0BenchBanner     asc 0D,'PGT36 R3B DEAD BENCH CODE',0D
                  asc 'No Tool225. 16K app / <=8K reads; MVN drain.',0D
                  asc 'D=snapshot  ESC=exit',0D,00
R0BenchFailMsg    asc 0D,'R0 transport read failed.',0D,00
R0SnapBytesMsg    asc 'R3B bytes=$',00
R0SnapTicksMsg    asc ' ticks=$',00
R0SnapQueueMsg    asc ' q=$',00
R0SnapQHighMsg    asc ' qhi=$',00
R0SnapHeadMsg     asc ' head=$',00
R0SnapTailMsg     asc ' tail=$',00
R0SnapErrMsg      asc ' err(cs/seq/win/hard)=$',00
R2SnapDupMsg     asc ' dup=$',00
R2SnapFutureMsg  asc ' fut=$',00
